...
###############################################################################
# Script cài đặt N8N (kèm Postgres, Redis, Docker, SSL thủ công hoặc tự động)
# Hỗ trợ domain chính và subdomain
###############################################################################
...
echo "======================================"
echo ""
read -p "Nhập domain chính (ví dụ: cogihay.xyz): " DOMAIN_NAME
...
if [ "$SSL_OPTION" = "1" ]; then
    echo ""
    echo "Cài đặt Certbot & lấy chứng chỉ Let's Encrypt cho $HOSTNAME ..."
    apt-get update -y
    apt-get install -y certbot python3-certbot-nginx

    systemctl stop nginx

    certbot certonly --standalone --non-interactive --agree-tos --register-unsafely-without-email -d "$HOSTNAME"
    CERT_PATH="/etc/letsencrypt/live/$HOSTNAME/fullchain.pem"
    KEY_PATH="/etc/letsencrypt/live/$HOSTNAME/privkey.pem"

    if [ ! -f "$CERT_PATH" ] || [ ! -f "$KEY_PATH" ]; then
        echo "Không lấy được SSL cho $HOSTNAME. Hãy kiểm tra lại domain hoặc cổng 80!"
        exit 1
    fi

    echo "Tạo thư mục lưu SSL cho Nginx: /etc/nginx/ssl/$HOSTNAME/"
    mkdir -p "/etc/nginx/ssl/$HOSTNAME/"

    cp "$CERT_PATH" "/etc/nginx/ssl/$HOSTNAME/certificate.crt"
    cp "$KEY_PATH" "/etc/nginx/ssl/$HOSTNAME/private.key"

    systemctl start nginx
    echo "SSL Let's Encrypt đã được cài đặt và cấu hình xong cho $HOSTNAME"
else
